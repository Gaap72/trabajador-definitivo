...
export default function TeacherPage() {
      ...
      const formatted = (profiles || []).map(p => {
        ...
        return {
          ...
          grades: {
            ...
            director_grade: backup[p.id]?.director_grade !== undefined ? Number(backup[p.id].director_grade) : (isLocalNewer ? localStudent.grades.director_grade : Number(dg)),
          },
  ...
  const toggleAttendance = async (sid: string) => {
    ...
    try {
      if (newStatus === 'ABSENT') {
        await supabase.from('attendance').upsert({ student_id: sid, date: selectedDate, status: 'ABSENT' }, { onConflict: 'student_id,date' })
      } else {
        await supabase.from('attendance').delete().eq('student_id', sid).eq('date', selectedDate)
      }
      await fetchData()
    } catch (err) {
  ...
  const handleJustifyAbsence = async () => {
    ...
    setIsSyncing(true)
    try {
      const { error } = await supabase.from('attendance').delete().eq('student_id', justifyModal.sid).eq('date', dateToJustify)
      ...
      alert('❌ Error: La fecha no coincide con una falta registrada.')
    } finally {
      setIsSyncing(false)
    }
  }
          ...
          <div className="bg-white rounded-[3.5rem] border border-slate-200 shadow-xl overflow-hidden relative">
                    ...
                    <tr key={s.id} className="hover:bg-slate-50/30">
                      ...
                      {activeTab === 'grades' ? (
                        <>
                          ...
                          <td className={`p-12 text-center bg-indigo-50 font-black text-4xl tracking-tighter ${Number(avg) >= 7 ? 'text-indigo-600' : 'text-red-500'}`}>{avg}</td>
                        </>
                      ) : (
                        <>
                          <td className="p-12 text-center"><button onClick={() => toggleAttendance(s.id)} className={`w-full px-6 py-4 rounded-2xl font-black uppercase text-[10px] transition-all ${attendanceMap[s.id] === 'ABSENT' ? 'bg-red-600 text-white shadow-xl' : 'bg-slate-100 text-slate-400 hover:text-indigo-600'}`}>{attendanceMap[s.id] === 'ABSENT' ? 'Falta Hoy' : 'Asistencia'}</button><button onClick={() => setJustifyModal({sid: s.id, name: s.full_name})} className="text-[9px] font-black text-indigo-500 hover:underline uppercase tracking-tighter">Justificar Falta</button></td>
                          <td className="p-12 text-center font-black text-2xl text-slate-300">{s.absences_count} / 12</td>
                        </>
                      )}
                      <td className="p-12 text-center">
                         <div className={`px-8 py-4 rounded-2xl font-black uppercase text-[10px] border-2 flex items-center justify-center gap-3 ${isFailing ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'}`}>{isFailing ? <AlertCircle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}{isFailing ? 'Reprobado' : 'Regular'}</div>
                      </td>
                    </tr>
                  )
                })}
            ...
            {(Object.keys(pendingGrades).length > 0 || (activeTab === 'attendance' && changedAttendance.size > 0)) && (
              <div className="fixed bottom-12 right-12 flex items-center gap-6 bg-slate-900 p-5 pl-12 rounded-[3rem] shadow-2xl border border-slate-700 z-50 animate-in slide-in-from-right-40 duration-700">
                 <div className="text-left"><p className="text-white font-black uppercase text-sm tracking-[0.2em] mb-1">Cambios en Boletas</p><p className="text-slate-500 text-[10px] font-bold">Confirma para sincronizar con la nube</p></div>
                 <button onClick={activeTab === 'grades' ? syncGradesWithCloud : syncAttendanceWithCloud} disabled={isSyncing} className="bg-indigo-600 text-white px-16 py-7 rounded-[2.5rem] font-black text-xl hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50 shadow-2xl flex items-center gap-4">{isSyncing ? <Loader2 className="h-7 w-7 animate-spin" /> : <Save className="h-7 w-7" />}{isSyncing ? 'CONECTANDO...' : 'GUARDAR AHORA'}</button>
              </div>
            )}
          </div>
        </div>
      ...
      {justifyModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
           <div className="bg-white max-w-lg w-full rounded-[3rem] shadow-2xl p-12 border border-slate-100 animate-in zoom-in-95 duration-300">
              <div className="text-center mb-8"><div className="inline-flex p-4 bg-indigo-50 rounded-2xl mb-4 text-indigo-600"><Shield className="h-8 w-8" /></div><h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Justificar Falta</h3><p className="text-slate-400 font-bold text-sm mt-2">Alumno: {justifyModal.name}</p></div>
              <div className="space-y-6">
                 <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2">Fecha de la Falta</label><input type="date" value={dateToJustify} onChange={(e) => setDateToJustify(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-2xl font-black text-slate-700 focus:border-indigo-600 outline-none transition-all" /></div>
                 <div className="flex gap-4"><button onClick={() => setJustifyModal(null)} className="flex-1 bg-slate-100 text-slate-400 py-5 rounded-2xl font-black uppercase text-xs hover:bg-slate-200 transition-all">Cancelar</button><button onClick={handleJustifyAbsence} disabled={!dateToJustify || isSyncing} className="flex-1 bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase text-xs hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 disabled:opacity-50">{isSyncing ? 'Procesando...' : 'Confirmar'}</button></div>
              </div>
           </div>
        </div>
      )}
